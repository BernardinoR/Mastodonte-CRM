...
} from "@shared/components/filter-bar/DateRangeFilterContent";
...
export function ClientTasks({ tasks, inlineProps, clientName }: ClientTasksProps) {
  ...
  const filteredTasks = useMemo(() => {
    ...
    if (selectedAssignees.length > 0) {
      result = result.filter(
        (task) => task.assignees?.some((a) => selectedAssignees.includes(a)) ?? false,
      );
    }
  ...
  const searchFilter = useSearchFilter<GlobalTask>(filteredTasks, (task, term) =>
    ...
    <div>
      <div className="mb-4 flex items-center justify-between">
        ...
        <Link
          ...
          className="flex items-center gap-1 text-sm text-[#2eaadc] hover:underline"
