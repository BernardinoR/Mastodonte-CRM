...
export function useInlineClientMeetings(options: UseInlineClientMeetingsOptions) {
  ...
  const defaultAssigneeName =
    defaultAssignee || currentUserData?.user?.name || "Rafael Bernardino Silveira";
  ...
  const [newMeetingStatus, setNewMeetingStatus] = useState<MeetingStatus>(
    MEETING_STATUS_OPTIONS[0],
  );
  ...
  const handleNewMeetingRowBlur = useCallback(
    (e: React.FocusEvent) => {
