    ...
    try {
      ...
      if (!user) {
        ...
        if (isFirstUser) {
          ...
          const validPendingRoles = publicMetadata.pendingRoles.filter((r: string) =>
          ...
          validRoles.includes(publicMetadata.pendingRole as UserRole)
        ) {
  ...
  app.post("/api/invitations", clerkAuthMiddleware, requireAdmin(), async (req, res) => {
    try {
      ...
      if (clerkCode === "form_identifier_exists") {
        return res
          .status(400)
          .json({ error: "Este email já possui uma conta ou convite pendente" });
      }
